...
class Value {
	...
	constructor(holder, key, value, parent) {
		...
		if (current_root) {
			current_root.values.set(value, this);
		}
	}
	...
	getReference() {
		...
		if (!this.is_registered) {
			this.id = current_root.duplicates.push(this) - 1;
			this.is_registered = true;
			this.reference = {dry: 'ref', value: this.id};
		}
...
class RootValue extends Value {
	 ...
	 * @param    {*}          value     The actual value to replace
	 */
	revive(holder, key, value, parent) {
		...
		if (type == 'object') {
			...
			if (typeof dry_type == 'string') {
				if (dry_type == 'ref' && value.value != null) {
					result = new RefValue(holder, key, value, parent);
				} else if (dry_type == 'root') {
					result = this;
				}
			}
		}
	...
	driedValue() {
		...
		if (!this.is_processed) {
			current_root = this;
			this.is_processed = true;
			this.real_root = this.createValueWrapper(null, null, this.value);
		...
		for (i = 0; i < current_root.duplicates.length; i++) {
			duplicate = current_root.duplicates[i];
	...
	undriedValue() {
		...
		if (!this.is_processed) {
			...
			this.real_root = current_root.createValueWrapper(null, null, value);
			current_root.values.set(value, this);
			this.replaced_value = this.real_root.undriedValue();
		}

		return this.real_root.replaced_value;
	}
}
...
class ObjectValue extends Value {
	...
	initialReplace(value) {
		...
		if (replace_again && value) {
			result = current_root.replace(null, null, value);
		}
	...
	simpleReplace(value) {
		...
		for (i = 0; i < keys.length; i++) {
			key = keys[i];
			new_value[key] = current_root.replace(value, key, value[key], this);
		}
	...
	initialRevive(value) {
		...
		for (i = 0; i < keys.length; i++) {
			key = keys[i];
			new_value[key] = current_root.revive(value, key, value[key], this);
		}
...
class ArrayValue extends ObjectValue {
	...
	simpleReplace(value) {
		...
		for (i = 0; i < length; i++) {
			new_value.push(current_root.replace(value, i, value[i], this));
		}
	...
	initialRevive(value) {
		...
		for (i = 0; i < length; i++) {
			new_value.push(current_root.revive(value, i, value[i], this));
		}
...
class RefValue extends Value {
	undriedValue() {
		return current_root.duplicates[this.value.value];
	}
}
