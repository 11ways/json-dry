...
function generateReviver(reviver, undry_paths) {
	...
	return function dryReviver(key, value) {
		...
		if (val_type === 'string') {
			...
			} else if (value[0] == '\\' && value[1] == 'x' && value[2] == 'e' && value[3] == '7') {
			...
			switch (value.dry) {
				...
				case 'toDry':
					...
					if (value.path) {
						constructor = fromPath(Blast.Globals, value.path);
					} else {
						if (value.namespace) {
							constructor = fromPath(Blast.Classes, value.namespace);
						} else {
...
module.exports.toObject  = toDryObject;
