	...
	undriedValue(holder, key) {
		...
		if (!this.is_processed) {
			...
			if (value.$refs && value.$root) {
				let length = value.$refs.length,
				    $refs = [],
				    i;
				
				for (i = 0; i < length; i++) {
					$refs.push(this.createReviverValueWrapper(null, null, value.$refs[i], this));
				}
...
class ObjectValue extends Value {
	...
	initialReplace(value) {
		...
		if (replace_again && result) {
			result = this.root.replace(null, null, result, this);
		}
	...
	initialRevive(value) {
		...
		for (i = 0; i < keys.length; i++) {
			key = keys[i];
			new_value[key] = this.root.revive(value, key, value[key], this, new_value);
		}
...
class ArrayValue extends ObjectValue {
	...
	initialRevive(value) {
		...
		for (i = 0; i < length; i++) {
			new_value.push(this.root.revive(value, i, value[i], this, new_value));
		}
...
class RefValue extends Value {
	undriedValue(holder, key) {
		...
		return value.undriedValue(holder, key);
	}
}
...
class UndrierValue extends Value {
	...
	undriedValue(holder, key) {
		...
		if (this.is_busy) {
			...
			this.placeholders.push([holder, key]);
...
class CustomReviverValue extends UndrierValue {
	...
	reviveWithReviver(value) {
		...
		if (constructor && typeof constructor.unDry === 'function') {
			let new_result = constructor.unDry(result);
			result.$replaced = new_result;
			result = new_result;
		}
