...
function createDryReplacer(root, replacer) {
	...
	return function dryReplacer(holder, key, value) {
			...
			case 'object':
				...
				if (seen_path) {
					...
					if (root.ultradry) {
						...
						if (root.replacements[seen_path] == null && !root.registers[seen_path]) {
							root.replacements[seen_path] = r_index++;

							let new_path = special_char + root.replacements[seen_path];
							root.registers[new_path] = true;

							value_paths.set(value, new_path);
							seen_path = new_path;
						}

						if (false) {
							let original = fromPath(root[''], seen_path, '~');

							console.log('Seen path:', seen_path);

							if (original != null) {
								let index = root.seens.push(seen_path) - 1,
								    old_path = seen_path;

								seen_path = special_char + index;
								value_paths.set(value, seen_path);

								root.replacements[old_path] = seen_path;
							} else {
								root.unfound[seen_path]
							}
						}
					}
		...
		if (was_root && root.ultradry) {
			let paths = [],
			    index,
			    key,
			    i;

			for (i = 0; i < root.seens.length; i++) {
				key = root.seens[i];
				root.seens[i] = fromPath(value, key, '~');
			}

			for (key in root.replacements) {
				paths.push([key, root.replacements[key]]);
			}

			paths.sort(function sortAb(a, b) {
			...
			for (i = 0; i < paths.length; i++) {
				...
				root.seens[index] = fromPath(value, key, '~');

				setPathByString(value, key, '~', '~' + index);
			}
 ...
 * @return   {Mixed}
 */
function fromPath(obj, path, separator) {
...
/**
 * Set something on the given string path
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    1.0.9
 * @version  1.0.9
 ...
 * @return   {Mixed}
 */
function setPathByString(obj, path, separator, value, force) {
	...
	return setPath(obj, path, value, force);
}
 ...
 * @return   {Object}
 */
function toDryObject(value, replacer, ultradry) {
 ...
 * @return   {Object}
 */
function stringify(value, replacer, space, ultradry) {
	return JSON.stringify(toDryObject(value, replacer, ultradry), null, space);
}
...
function parse(object, reviver) {
	...
	if (object.dry === 'ultradry') {
		let parsed = parse(object.register, reviver);
