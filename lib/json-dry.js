...
class RootValue {
	...
	undriedValue() {
		...
		if (!this.is_processed) {
			...
			this.real_root = this.createValueWrapper(null, null, value, this);
...
class ObjectValue extends Value {
	...
	initialReplace(value) {
		...
		if (replace_again && value) {
			result = this.root.replace(null, null, value, this);
		}
	...
	initialRevive(value) {
		...
		for (i = 0; i < keys.length; i++) {
			key = keys[i];
			new_value[key] = this.root.revive(value, key, value[key], this);
		}
...
class ArrayValue extends ObjectValue {
	...
	simpleReplace(value) {
		...
		for (i = 0; i < length; i++) {
			new_value.push(this.root.replace(value, i, value[i], this));
		}
	...
	initialRevive(value) {
		...
		for (i = 0; i < length; i++) {
			new_value.push(this.root.revive(value, i, value[i], this));
		}
...
class RefValue extends Value {
	undriedValue() {
		return this.root.duplicates[this.value.value];
	}
}
