...
class Drier {
	...
	constructor(root, replacer) {
		drier = this;
		this.root = new RootValue(null, null, root);
	...
	start() {
		drier = this;
		let result = this.root.driedValue();
		drier = null;
		return result;
	}
}
...
class Value {
	...
	getReference() {
		...
		if (!this.is_registered) {
			this.id = drier.duplicates.push(this) - 1;
...
class RootValue extends Value {
	...
	driedValue() {
		...
		if (!this.is_processed) {
			this.is_processed = true;
			this.real_root = drier.createValueWrapper(null, null, this.value);
			drier.values.set(this.value, this);
			this.replaced_value = this.real_root.driedValue();
		} else {
		...
		for (i = 0; i < drier.duplicates.length; i++) {
			duplicate = drier.duplicates[i];
			$dry.push(duplicate.replaced_value);
		}

		this.replaced_value.$dry = $dry;
...
class ObjectValue extends Value {
	...
	initialReplace(value) {
		...
		if (replace_again && value) {
			result = drier.replace(null, null, value);
		}
	...
	simpleReplace(value) {
		...
		for (i = 0; i < keys.length; i++) {
			key = keys[i];
			new_value[key] = drier.replace(value, key, value[key], this);
		}
...
class ArrayValue extends ObjectValue {
	...
	simpleReplace(value) {
		...
		for (i = 0; i < length; i++) {
			new_value.push(drier.replace(value, i, value[i], this));
		}
